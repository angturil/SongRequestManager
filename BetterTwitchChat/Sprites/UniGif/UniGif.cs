...
public static partial class UniGif
{
    ...
    public static IEnumerator GetTextureListCoroutine(
        byte[] bytes,
        Action<List<GifSprite>, int, int, int, string, string> callback,
        Action<List<GifSprite>, string> earlyCallback,
        string messageIndex,
        string emoteIndex,
        ...
        bool debugLog = false)
    {
        ...
        if (SetGifData(bytes, ref gifData, debugLog) == false)
        {
            ...
            if (callback != null)
            {
                callback(null, loopCount, width, height, messageIndex, emoteIndex);
            }
            yield break;
        }

        // Decode to textures from GIF data
        List<GifSprite> gifTexList = null;
        yield return DecodeTextureCoroutine(bytes, result => gifTexList = result, earlyCallback, filterMode, wrapMode, emoteIndex);
        ...
        if (callback != null)
        {
            callback(gifTexList, loopCount, width, height, messageIndex, emoteIndex);
        }
